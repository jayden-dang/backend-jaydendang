...
pub async fn get<MC, O>(_ctx: &Ctx, mm: &ModelManager, id: i64) -> Result<O>
where
    MC: DMC,
    O: HasSeaFields + for<'a> FromRow<'a, PgRow> + Send + Unpin,
{
    ...
    O: HasSeaFields + for<'a> FromRow<'a, PgRow> + Send + Unpin,
{
    ...
    list::<MC, O, F>(ctx, mm, filter, Some(list_options))
        .await
...
pub async fn update<MC, O>(ctx: &Ctx, mm: &ModelManager, id: i64, data: O) -> Result<()>
where
    MC: DMC,
    O: HasSeaFields,
{
