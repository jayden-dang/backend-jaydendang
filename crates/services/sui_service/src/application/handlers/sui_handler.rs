...
impl<R: SuiRepository> SuiHandler<R> {
  ...
  async fn create_repository_with_gas_station(state: AppState) -> Result<SuiRepositoryImpl> {
    let config = Config::from_env().map_err(|e| crate::error::Error::Internal(format!("Config error: {}", e)))?;
    
    if let Some(sponsor_addr_str) = &config.sui.sponsor_address {
      let sponsor_address = SuiAddress::from_str(sponsor_addr_str)
        .map_err(|e| crate::error::Error::InvalidRequest(format!("Invalid sponsor address '{}': {}", sponsor_addr_str, e)))?;
      ...
      let rpc_url = match config.sui.env.to_lowercase().as_str() {
        ...
        _ => return Err(crate::error::Error::Internal(format!("Unsupported Sui environment: {}", config.sui.env))),
      };
      
      // Check if we have private key for full sponsor functionality
      if let Some(private_key) = &config.sui.sponsor_private_key {
        SuiRepositoryImpl::with_gas_station_and_key(state, rpc_url, sponsor_address, private_key, max_gas_budget).await
      } else {
        SuiRepositoryImpl::with_gas_station(state, rpc_url, sponsor_address, max_gas_budget).await
      }
    } else {
      Err(crate::error::Error::Internal("Gas station requires SUI_SPONSOR_ADDRESS environment variable".to_string()))
    }
  }
  ...
  pub async fn debug_config() -> Result<Json<Value>> {
    ...
    let debug_info = json!({
      ...
      "required_env_vars": {
        ...
        "SUI_ENV": std::env::var("SUI_ENV").unwrap_or("NOT_SET".to_string()),
      },
  ...
  pub async fn gas_pool_status(
    State(state): State<AppState>,
  ) -> Result<Json<GasPoolStatus>> {
    let repository = Self::create_repository_with_gas_station(state).await?;
    let use_cases = SuiUseCases::new(repository);
  ...
  pub async fn test_gas_station(State(state): State<AppState>) -> Result<Json<Value>> {
    ...
    if let Some(sponsor_addr_str) = &config.sui.sponsor_address {
      match SuiAddress::from_str(sponsor_addr_str) {
        Ok(sponsor_address) => {
          ...
          tracing::info!("Attempting to create gas station with sponsor: {}, rpc: {}", sponsor_address, rpc_url);
          
          match SuiRepositoryImpl::with_gas_station(state, rpc_url, sponsor_address, max_gas_budget).await {
            Ok(repository) => {
