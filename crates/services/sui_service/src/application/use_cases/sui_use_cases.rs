  ...
  pub async fn sponsor_transaction(&self, request: SponsorRequest) -> Result<SponsorResponse> {
        ...
        .await 
    {
        Ok((transaction, digest)) => {
            ...
            let gas_budget = request.transaction_data.gas_data.budget.parse::<u64>()
              .unwrap_or(10000000); // Default gas budget
            
            // Log the sponsored transaction
            if let Err(e) = self.repository.log_sponsored_transaction(&user_address, gas_budget).await {
            ...
            Ok(SponsorResponse {
                ...
                message: Some(format!("Transaction executed successfully on Sui network. Digest: {}", digest)),
            })
        },
