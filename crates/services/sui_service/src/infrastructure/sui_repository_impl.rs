...
use sui_types::{
  ...
  crypto::{Signature, Ed25519SuiSignature, SuiSignature, PublicKey, Signer},
  gas_coin::GasCoin,
};
use sui_keys::keystore::{AccountKeystore, InMemKeystore};
use fastcrypto::{traits::{ToFromBytes, KeyPair}, ed25519::Ed25519KeyPair};
...
impl SuiRepositoryImpl {
  ...
  pub async fn with_gas_station_and_key(
    ...
    keystore.add_key(None, sui_keypair)
...
impl SuiRepository for SuiRepositoryImpl {
  ...
  async fn get_available_gas(&self, required_budget: u64) -> Result<ObjectID> {
    let gas_station = self.gas_station
      .as_ref()
  ...
  async fn sponsor_transaction(
    ...
    let keystore = self.sponsor_keystore
      .as_ref()
      .ok_or_else(|| Error::Internal("Sponsor keystore not available. Private key required for transaction sponsoring.".to_string()))?;
    ...
    let mut sponsored_tx_data = tx_data.clone();
    ...
    if user_signature.len() != 64 {
        tracing::error!("Invalid user signature length: {} bytes (expected 64)", user_signature.len());
        return Err(Error::Internal("Invalid user signature length. Expected 64 bytes for Ed25519 signature.".to_string()));
    }
    ...
    let response = gas_station.sui_client
        .quorum_driver_api()
  ...
  async fn check_rate_limit(&self, user_address: &SuiAddress) -> Result<bool> {
    ...
    let count: Option<i32> = conn.get(&key)
      .await
      .map_err(|e| Error::Internal(e.to_string()))?;
